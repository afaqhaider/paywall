...
The Customer Portal (`apps/web/src/app/portal/**`) is the self-service area for **end customers** — people or accounts who have purchased a subscription/license to an application built on the SS Zentronics Platform. It is not used by the developers/vendors who built that application (see the Developer Portal) or by platform administrators (see the Admin Console).
